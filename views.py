...
class NameModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        # modal の処理は時間がかかる可能性があるため先に ACK
...
class StorageSelectView(discord.ui.View):
    ...
    async def retrieve_item(self, interaction: discord.Interaction):
        ...
        if not item_data:
            await interaction.followup.send("⚠️ アイテムが見つかりません。", ephemeral=True)
            return
        ...
        if success:
            ...
            # interaction は既に defer 済みなので編集は message.edit を使う
            try:
            ...
            await interaction.followup.send("⚠️ アイテムの取り出しに失敗しました。", ephemeral=True)
...
class ResetFinalConfirmView(discord.ui.View):
    ...
    async def final_confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # interaction はすでに defer 済みなので編集は message.edit または followup を使用
        try:
            await interaction.message.edit(embed=embed, view=None)
        except Exception:
            await interaction.followup.send(embed=embed)
...
class SpecialEventView(View):
    ...
    async def special_enemy(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # interaction は defer 済み -> 編集は message.edit
        await interaction.message.edit(embed=embed, view=None)
    ...
    async def store_item(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if notification_channel:
                success_count = db.get_success_count(interaction.user.id)
                notify_embed = discord.Embed(
                    title="🎉 ラスボス討伐成功！",
                    description=f"**{interaction.user.name}** がラスボスを討伐し、**{selected_item}** を倉庫に保管した！\n\n**クリア回数**: {success_count}回",
                    color=discord.Color.gold()
                )
...
class FinalBossBattleView(View):
    ...
    async def use_skill(self, interaction: discord.Interaction):
        ...
        if db.is_mp_stunned(interaction.user.id):
            ...
            return await interaction.response.send_message("⚠️ MP枯渇で行動不能！\n『嘘だろ？！』\n次のターンから行動可能になります。", ephemeral=True)
        ...
        if not skill_info:
            return await interaction.followup.send("⚠️ スキル情報が見つかりません。", ephemeral=True)
        ...
        if current_mp < mp_cost:
            return await interaction.followup.send(f"⚠️ MPが足りません！（必要: {mp_cost}, 現在: {current_mp}）", ephemeral=True)
        
        if not db.consume_mp(interaction.user.id, mp_cost):
            return await interaction.followup.send("⚠️ MP消費に失敗しました。", ephemeral=True)
        ...
        if armor_result["evaded"]:
            ...
            if armor_result["counter_damage"] > 0:
                ...
                if self.boss["hp"] <= 0:
                    ...
                    embed.add_field(
                        name="🔄 もう1度周回しますか？", 
                        value="距離・所持アイテムがリセットされ、新たな冒険が始まります！\n\n※シークレットアイテムなども失われます", 
                        inline=False
                    )
                    self.disable_all_items()
                    
                    # 周回確認Viewを表示
                    confirm_view = LoopConfirmView(interaction.user.id, self.ctx, self.user_processing, self.boss_stage)
                    await interaction.message.edit(embed=embed, view=confirm_view)
                    return
...
class BossBattleView(View):
    ...
    async def use_skill(self, interaction: discord.Interaction):
        ...
        if db.is_mp_stunned(interaction.user.id):
            db.set_mp_stunned(interaction.user.id, False)
            return await interaction.response.send_message("⚠️ MP枯渇で行動不能！次のターンから行動可能になります。", ephemeral=True)
        
        # heavy path -> ACK early
        await interaction.response.defer()
    ...
    async def run(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # defer early because DB update follows
        await interaction.response.defer()
    ...
    async def use_item(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def item_select_callback(select_interaction: discord.Interaction):
            ...
            # ACK early because we do DB updates and edits
            ...
            item_info = game.get_item_info(item_name)
            if not item_info:
                return await select_interaction.followup.send("アイテム情報が見つかりません。", ephemeral=True)
            ...
            await self.update_embed(text)
            await self.message.edit(view=self)
            # select_interaction was deferred; no need to defer again
...
class BlacksmithView(discord.ui.View):
    ...
    def get_embed(self):
        embed = discord.Embed(
            title="🔨 鍛冶屋",
            description="「素材を使って強力な装備を作ることができるぞ」\n\n所持素材:",
            color=discord.Color.blue()
        )
    ...
    async def craft_callback(self, interaction: discord.Interaction):
        ...
        player = get_player(interaction.user.id)
        if not player:
            return await interaction.followup.send("プレイヤーデータが見つかりません。", ephemeral=True)
...
class MaterialMerchantView(discord.ui.View):
    ...
    async def sell_callback(self, interaction: discord.Interaction):
        ...
        # defer because DB updates will be performed
        await interaction.response.defer()
