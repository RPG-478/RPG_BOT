...
def check_death_triggers(user_id):
    """
    ...
    # 1. 特定敵への死亡回数チェック
    story_trigger = check_enemy_death_count_triggers(user_id)
    if story_trigger:
        return story_trigger
    
    # 2. 連続死亡パターンチェック
    pattern_trigger = check_death_pattern_triggers(user_id)
    if pattern_trigger:
        return pattern_trigger
    
    # 3. 称号解放チェック
    title_trigger = check_title_triggers(user_id)
    if title_trigger:
...
def check_enemy_death_count_triggers(user_id):
    """特定の敵に○回殺された時のトリガー"""
    ...
    for story_id, story_data in DEATH_STORIES.items():
        ...
        if trigger_type == "enemy_count":
            ...
            # 既に表示済みかチェック
            if db.get_story_flag(user_id, story_id):
                continue
            
            # 死亡回数チェック
            death_count = db.get_death_count_by_enemy(user_id, enemy_name)
            
            if death_count >= required_count:
                # フラグを立てて返す
                db.set_story_flag(user_id, story_id)
                return {
                    "type": "story",
                    "story_id": story_id,
                    "data": story_data
...
def check_death_pattern_triggers(user_id):
    """連続死亡パターンのトリガー"""
    ...
    for pattern_id, pattern_data in DEATH_PATTERNS.items():
        ...
        # パターンマッチチェック
        if db.check_death_pattern(user_id, pattern):
            db.set_story_flag(user_id, story_id)
            return {
                "type": "story",
                "story_id": story_id,
...
def check_title_triggers(user_id):
    """称号解放トリガー"""
    ...
    for title_id, title_data in TITLES.items():
        ...
        if condition_type == "enemy_deaths":
            ...
            if death_count >= required_count:
                # 称号付与
                db.add_title(user_id, title_id, title_data.get("name"))
                return {
                    "type": "title",
                    "title_id": title_id,
                    "data": title_data
...
def get_death_summary(user_id):
    """死亡統計サマリーを取得"""
    stats = db.get_death_stats(user_id)
    player = db.get_player(user_id)
    total_deaths = player.get("total_deaths", 0) if player else 0
...
def get_death_story_progress(user_id):
    """死亡ストーリーの進行状況を取得"""
